...
sub getCurrentPlayList {
	my $client = shift;
	my $currentPlaying = eval { Plugins::DynamicPlaylists3::Plugin::getCurrentPlayList($client) };
	if ($@) {
		warn("SQLPlayList: Error getting current playlist from DynamicPlaylists3 plugin: $@\n");
	}
	if($currentPlaying) {
...
sub initPlayLists {
	...
	if(defined($client)) {
		# We need to make sure the playlists in DynamicPlaylists3 plugin is re-read
...
sub handleWebList {
	...
	if(!UNIVERSAL::can("Plugins::DynamicPlaylists3::Plugin","getCurrentPlayList")) {
		$params->{'pluginSQLPlayListError'} = "ERROR!!! Cannot find DynamicPlaylists3 plugin, please make sure you have installed and enabled at least DynamicPlaylists3 3.x"
	}
